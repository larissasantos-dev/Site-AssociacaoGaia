document.addEventListener('DOMContentLoaded', () => {
    ...
    // O login.html salva o usuário logado em 'gaia_usuario' (JSON),
    // com o campo tipoUsuario vindo do UsuarioResponseDTO do backend.
    const usuarioSalvo = localStorage.getItem('gaia_usuario');
    let tipoUsuarioLogado = null;

    if (usuarioSalvo) {
        try {
            tipoUsuarioLogado = JSON.parse(usuarioSalvo).tipoUsuario;
        } catch (erro) {
            // Se o valor salvo estiver corrompido, trata como
            // "não logado" em vez de quebrar a página inteira.
